...
Hessian eigenvalue tracking (Appendix~\ref{app:hessian}) reveals that the plateau is a \emph{saddle point} ($\lambda_\text{min} < 0$ throughout), not a local minimum---ruling out spinodal decomposition.
The most negative eigenvalue is ${\sim}10^3{\times}$ smaller than $\lambda_\text{max}$, explaining why the optimizer random-walks for many steps before finding the descent channel.
At the transition, $\lambda_\text{max}$ spikes $10$--$20{\times}$ as the model descends through a narrow, steep canyon.
...
Weight norm dynamics partially confirm deep grokking predictions~\citep{gromov2024grokking}: while total $\|W\|_F$ grows monotonically, the layer containing L1H3 shows non-monotonic behavior (Appendix~\ref{app:weight-norms}).

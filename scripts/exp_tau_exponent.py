...
K_RESTRICTED = [10, 20, 36]  # Table 2's range
SEED = 42
LR = 1e-3
MAX_STEPS = 50000

# Try these naming patterns to find existing runs
NAME_PATTERNS = [
    "landauer_dense_k{k}",
    "k{k}_lr1e-3",
    "exponent_K{k}",
]
# Canonical name for new runs
NEW_NAME = "exponent_K{k}"


def find_existing_run(k):
    """Check if a run already exists under any naming convention."""
...
# ── Check existing runs & build jobs for missing ones ──────────────────────
...
# Restricted range (Table 2's K values)
...
if tau_data:
    ...
    if "restricted" in fit_results and fit_results["restricted"]["delta"] is not None:
        ...
        ax.loglog(k_fit, np.exp(log_c_r) * k_fit ** d_r, ":",
                  ...
                  label=f"K∈{{10,20,36}}: δ={d_r:.2f}")

        # Highlight restricted points
